...
class Create_content_page():
    ...
    def read_settings_json(self, path):
        ...
        self.title_font_size: str = settings["title"]["font_size"]
        ...
        self.content_style_use_two_columns: str = settings["content"]["style"]["use_two_columns"]
    ...
    def get_text_size(self, text: str, font: ImageFont.ImageFont) -> tuple:
        text = str(text)
        return(ImageFont.ImageFont.getsize(font, text)[0])
    ...
    def resize(self, text: str, font_path: str, font_size: int, min_font_size: int, max_width: int):
        font = ImageFont.truetype(font_path, size=font_size)
        if max_width > self.get_text_size(text, font)[0]:
            return [text, font]
        else:
            for iter in range(font_size - min_font_size):
                font = ImageFont.truetype("v.otf", size=font_size - iter)
                if max_width > self.get_text_size(text, font)[0]:
                    return [text, font]
    ...
    def draw_content(self, resolution:tuple):
        ...
        if block_width > resolution[0] - self.page_left_margin - self.page_right_margin:
            block_width = resolution[0] - self.page_left_margin - self.page_right_margin
        ...
        for chapter in self.chapters:
            ...
            author, author_font = self.resize(author, self.content_author_font, self.content_author_font_size, self.content_author_min_font_size, block_width-page_block)
            autor_width, autor_height = self.get_text_size(author, author_font)
            self.draw_text.multiline_text(
                (left_border+block_width-autor_width-page_block, self.temp_height), author, font=author_font, fill=autor_fill)
            self.temp_height += autor_height + self.space_author_to_title

            page_font = ImageFont.truetype(
                self.content_page_number_font, self.content_page_number_font_size)
            page_width, page_height = self.get_text_size(pages, page_font)
            self.draw_text.text((left_border+block_width-page_width,
                                 self.temp_height), pages, font=page_font, fill=page_num_fill)
            ...
            self.draw_text.multiline_text(
                (left_border+block_width-title_width-page_block, self.temp_height), title, font=title_font, fill=title_fill)
            self.temp_height += title_height + \
